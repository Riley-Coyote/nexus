...
// Conditional import for client-side only - prevents server-side boundary violations
let getCurrentUser: (() => any) | null = null;
if (typeof window !== 'undefined') {
  try {
    getCurrentUser = require('../auth/AuthContext').getCurrentUser;
  } catch (error) {
...
// Safe wrapper for getCurrentUser
const safeGetCurrentUser = (): any => {
...
export class DataService {
  ...
  async submitEntry(content: string, type: string, isPublic: boolean, mode: 'logbook' | 'dream', currentUserId?: string): Promise<StreamEntry> {
    await this.initializeData();
    
    // Determine effective user ID (with fallback for backward compatibility)
    const effectiveUserId = currentUserId ?? 
      safeGetCurrentUser()?.id;
    
    // Ensure we have the authenticated user
    let currentUser = effectiveUserId ? null : (getCurrentUser ? safeGetCurrentUser() : null);
    ...
    // Use either the passed user ID or the current user's ID
  ...
  async resonateWithEntry(entryId: string, currentUserId?: string): Promise<boolean> {
    await this.initializeData();
    ...
    try {
      ...
      const newState = await this.database.toggleUserResonance(effectiveUserId, entryId);
      ...
      this.updateUserInteractionStateInCache(effectiveUserId, entryId, 'resonance', newState);
      
      // SYNC: Also update userInteractionService cache
      userInteractionService.updateUserInteractionState(effectiveUserId, entryId, 'resonance', newState);
  ...
  async amplifyEntry(entryId: string, currentUserId?: string): Promise<boolean> {
    await this.initializeData();
    ...
    try {
      ...
      // Determine effective user ID (with fallback for backward compatibility)
      const effectiveUserId = currentUserId ?? 
        (typeof window !== 'undefined' ? safeGetCurrentUser()?.id : undefined);
      
      if (!effectiveUserId) throw new Error('User not authenticated');

      const newState = await this.database.toggleUserAmplification(effectiveUserId, entryId);
      
      // OPTIMIZATION: Update both caches immediately without full refresh
      const delta = newState ? 1 : -1;
      this.updateInteractionCountInCache(entryId, 'amplification', delta);
      this.updateUserInteractionStateInCache(effectiveUserId, entryId, 'amplification', newState);
      
      // SYNC: Also update userInteractionService cache
      userInteractionService.updateUserInteractionState(effectiveUserId, entryId, 'amplification', newState);
  ...
  async createBranch(parentId: string, content: string, currentUserId?: string): Promise<void> {
    await this.initializeData();
  ...
  async getDirectChildren(parentId: string, currentUserId?: string): Promise<StreamEntry[]> {
    await this.initializeData();
    ...
    try {
      ...
      const enriched = await this.enrichEntriesWithInteractions(children, effectiveUserId);
      return await this.enrichEntriesWithUserContext(enriched, effectiveUserId);
    } catch (error) {
  ...
  async getEntryDetailsWithContext(entryId: string, currentUserId?: string): Promise<{
    ...
    const [currentEntryWithStates] = await (this.database.getEntriesByIdsWithUserStates
      ? await this.database.getEntriesByIdsWithUserStates([entryId], effectiveUserId)
      : []);
    ...
    if (currentEntryWithStates.parentId) {
      const [parentWithStates] = await (this.database.getEntriesByIdsWithUserStates
        ? await this.database.getEntriesByIdsWithUserStates([currentEntryWithStates.parentId], effectiveUserId)
        : []);
      if (parentWithStates) {
  ...
  async getPosts(options: {
    ...
    currentUserId?: string; // For interaction states
    ...
    try {
      ...
      switch (mode) {
        ...
        case 'resonated':
          ...
          entries = await this.getResonatedEntries(effectiveUserId, safePage, safeLimit);
          break;
        case 'amplified':
          if (!effectiveUserId) return [];
          entries = await this.getAmplifiedEntries(effectiveUserId);
          break;
        case 'profile':
          if (!userId) return [];
  ...
  async getPostsWithUserStates(options: {
    ...
    currentUserId?: string; // For backward compatibility
    sortBy?: 'timestamp' | 'interactions';
    sortOrder?: 'asc' | 'desc';
    filters?: {
      type?: string;
    ...
    if (!this.database.getEntriesWithUserStates) {
      ...
      return this.getPosts({ ...options, currentUserId });
    }
